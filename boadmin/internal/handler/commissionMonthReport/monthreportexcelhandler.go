...
import (
	"com.copo/bo_service/boadmin/internal/logic/commissionMonthReport"
...
func MonthReportExcelHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.CommissionMonthReportExcelRequest
		...
		l := commissionMonthReport.NewMonthReportExcelLogic(r.Context(), ctx)
		buffer, err := l.MonthReportExcel(&req)
		if err != nil {
			response.Json(w, r, err.Error(), nil, err)
		} else {
			fileName := "CommissionMonthReport" + time.Now().Format("20060102150405") + ".zip"
