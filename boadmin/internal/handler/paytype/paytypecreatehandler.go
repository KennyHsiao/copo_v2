...
func PayTypeCreateHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req1 types.PayTypeCreateRequest
		var req types.PayTypeCreate
		if err := r.ParseMultipartForm(32 << 20); err != nil {
			logx.Error(err)
			response.Json(w, r, response.FAIL, nil, nil)
			return
		}
		...
		logx.Info("MultipartForm.Value: ", r.MultipartForm.Value)
		...
		l := paytype.NewPayTypeCreateLogic(r.Context(), ctx)
		err := l.PayTypeCreate(req)
		if err != nil {
			logx.Error("支付類型新增錯誤: ", err.Error())
