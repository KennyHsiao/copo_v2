...
func MerchantExportListViewExcelHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.MerchantQueryListViewRequestX
		...
		l := merchant.NewMerchantExportListViewExcelLogic(r.Context(), ctx)
		xlsx, err := l.MerchantExportListViewExcel(req)

		if err != nil {
			response.Json(w, r, err.Error(), nil, err)
		} else {
			fileName := "merchant" + time.Now().Format("20060102150405") + ".xlsx"
