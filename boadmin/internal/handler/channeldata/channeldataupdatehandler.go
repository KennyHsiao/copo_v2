...
func ChannelDataUpdateHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.ChannelDataUpdateRequest
		logx.Info("渠道更新请求参数:{}", req)

		span := trace.SpanFromContext(r.Context())
		defer span.End()

		if err := httpx.ParseJsonBody(r, &req); err != nil {
			logx.Error("更新请求参数格式错误:{}", err)
		...
		l := channeldata.NewChannelDataUpdateLogic(r.Context(), ctx)
		err := l.ChannelDataUpdate(req)
		if err != nil {
			logx.Error("渠道更新错误: ", err)
