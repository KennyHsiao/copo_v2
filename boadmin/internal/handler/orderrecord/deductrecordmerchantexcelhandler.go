...
func DeductRecordMerchantExcelHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		l := orderrecord.NewDeductRecordMerchantExcelLogic(r.Context(), ctx)
		xlsx, err := l.DeductRecordMerchantExcel(req)
		if err != nil {
			response.Json(w, r, err.Error(), nil, err)
		} else {
			fileName := "PaymentReport" + time.Now().Format("20060102150405") + ".xlsx"
