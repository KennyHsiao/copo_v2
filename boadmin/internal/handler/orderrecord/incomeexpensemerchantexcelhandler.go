...
func IncomeExpenseMerchantExcelHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.IncomeExpenseQueryRequestX
		...
		l := orderrecord.NewIncomeMerchantExcelLogic(r.Context(), ctx)
		xlsx, err := l.IncomeMerchantExcel(&req)
		if err != nil {
			response.Json(w, r, err.Error(), nil, err)
		} else {
			fileName := "IncomeExpenseReport" + time.Now().Format("20060102150405") + ".xlsx"
