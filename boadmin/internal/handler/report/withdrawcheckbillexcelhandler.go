...
func WithdrawCheckBillExcelHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.WithdrawCheckBillQueryRequestX
		...
		l := report.NewWithdrawCheckBillExcelLogic(r.Context(), ctx)
		xlsx, err := l.WithdrawCheckBillExcel(&req)
		if err != nil {
			response.Json(w, r, err.Error(), nil, err)
		} else {
			fileName := "WithdrawReconcileReport" + time.Now().Format("20060102150405") + ".xlsx"
