...
func PayCheckBillExcelHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		l := report.NewPayCheckBillExcelLogic(r.Context(), ctx)
		xlsx, err := l.PayCheckBillExcel(&req)
		if err != nil {
			response.Json(w, r, err.Error(), nil, err)
		} else {
			fileName := "PayReconcileReport" + time.Now().Format("20060102150405") + ".xlsx"
