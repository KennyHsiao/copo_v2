...
func IncomReportExcelHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.IncomReportMonthQueryRequest
		...
		l := report.NewIncomReportExcelLogic(r.Context(), ctx)
		xlsx, err := l.IncomReportExcel(&req)
		if err != nil {
			response.Json(w, r, err.Error(), nil, err)
		} else {
			fileName := "EarningsReport" + time.Now().Format("20060102150405") + ".xlsx"
