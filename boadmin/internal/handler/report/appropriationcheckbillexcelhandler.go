...
func AppropriationCheckBillExcelHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.AppropriationCheckBillQueryRequest
		...
		l := report.NewAppropriationCheckBillExcelLogic(r.Context(), ctx)
		xlsx, err := l.AppropriationCheckBillExcel(&req)
		if err != nil {
			response.Json(w, r, err.Error(), nil, err)
		} else {
			fileName := "AppropriationReconcileReport" + time.Now().Format("20060102150405") + ".xlsx"
