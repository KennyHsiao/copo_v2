...
func MerchantReportQueryExcelHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.MerchantReportQueryRequestX
		...
		l := report.NewMerchantReportQueryExcelLogic(r.Context(), ctx)
		err := l.MerchantReportQueryExcel(&req)
		if err != nil {
			response.Json(w, r, err.Error(), nil, err)
		} else {
			//fileName := "VendorReport" + time.Now().Format("20060102150405") + ".xlsx"
