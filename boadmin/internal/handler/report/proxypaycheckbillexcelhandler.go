...
func ProxyPayCheckBillExcelHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		l := report.NewProxyPayCheckBillExcelLogic(r.Context(), ctx)
		xlsx, err := l.ProxyPayCheckBillExcel(&req)
		if err != nil {
			response.Json(w, r, err.Error(), nil, err)
		} else {
			fileName := "ProxyReconciles" + time.Now().Format("20060102150405") + ".xlsx"
