...
func (l *MerchantChannelRateConfigureLogic) MerchantChannelRateConfigure(req types.MerchantChannelRateConfigureRequest) (err error) {

	//用于判断是否为商户使用此API
	jwtMerchantCode := l.ctx.Value("merchantCode").(string)
	isUsedByMerchant := len(jwtMerchantCode) > 0
	...
	err = l.svcCtx.MyDB.Transaction(func(db *gorm.DB) error {
		...
		// 取得渠道
		if err = db.Table("ch_channels").Where("code = ? ", req.ChannelCode).Take(&channelData).Error; err != nil {
		...
		req.CurrencyCode = channelData.CurrencyCode
		...
		if isEnable, err := model.NewMerchantCurrency(l.svcCtx.MyDB).IsEnableDisplayPtBalance(req.MerchantCode, req.CurrencyCode); err != nil {
			...
			// 已启用子钱包 需指定子钱包
			return errorz.New("需指定子钱包")
		} else if !isEnable && req.MerchantPtBalanceId != 0 {
			// 未启用子钱包 不能指定子钱包
			return errorz.New("不能指定子钱包")
		}

		// 檢查費率
		if err = verificationFee(db, req, merchant, isUsedByMerchant); err != nil {
			return err
		}
...
func (l *MerchantChannelRateConfigureLogic) configure(db *gorm.DB, req types.MerchantChannelRateConfigureRequest, merchant *types.Merchant) (err error) {
	...
	if merchant.BillLadingType == "0" { //  若商戶為單指定時
		if merchantChannelRateConfigure.Designation == "1" { // 若本次為指定渠道
			// 若輪巡關閉時 且 本次為指定渠道 則 同一幣別&支付類型只能指定一個渠道支付 (把其他渠道支付改為非指定並去除代碼)
		...
		// 若商戶為多指定時開啟時 則 同一幣別&支付類型代碼不可重複 (把重複代碼的改為非指定 並去除代碼)
		if err = db.Table("mc_merchant_channel_rate").
			Where("merchant_code = ?", req.MerchantCode).
			Where("pay_type_code = ? ", req.PayTypeCode).
			Where("currency_code = ? ", req.CurrencyCode).
	...
	if merchantChannelRate, err = model.NewMerchantChannelRate(db).GetByMerchantCodeAndChannelPayTypeCode(req.MerchantCode, req.ChannelPayTypesCode); errors.Is(err, gorm.ErrRecordNotFound) {
		...
		err = db.Select("MerchantCode", "ChannelPayTypesCode", "ChannelCode", "PayTypeCode",
			"Designation", "DesignationNo", "Fee", "HandlingFee", "CurrencyCode", "MerchantPtBalanceId").
		...
		if err = db.Select("MerchantCode", "ChannelPayTypesCode", "ChannelCode", "PayTypeCode",
			"Designation", "DesignationNo", "Fee", "HandlingFee", "CurrencyCode", "MerchantPtBalanceId").
...
func verificationFee(db *gorm.DB, req types.MerchantChannelRateConfigureRequest, merchant *types.Merchant, isUsedByMerchant bool) (err error) {
	...
	if merchant.AgentParentCode != "" {
		...
		if merchant.RateCheck == "1" || isUsedByMerchant {
			if err != nil {
				return errorz.New(response.DATABASE_FAILURE, err.Error())
	...
	} else if req.Fee < channelPayType.Fee && (merchant.RateCheck == "1" || isUsedByMerchant) {
		return errorz.New(response.SETTING_MERCHANT_RATE_MIN_CHARGE_ERROR, "配置商户费率低消不可低于渠道成本费率低消")
	} else if req.HandlingFee < channelPayType.HandlingFee && (merchant.RateCheck == "1" || isUsedByMerchant) {
	...
	if merchant.AgentLayerCode != "" {
		...
		for _, subAgentMerchant := range subAgentMerchants {
			if subAgentMerchant.RateCheck == "0" && !isUsedByMerchant {
